...
export class MangaSyncService {
  ...
  async sync() {
    console.log('Syncing manga...');
    const localMediaItems = await db.mediaItems.where('mediaTypeId').equals(2).toArray();
    ...
    for (const local of localMediaItems) {
      ...
      if (!remote && !local.supabaseId && local.externalId) {
        remote = remoteMediaItems?.find(r => r.external_id === local.externalId && r.media_type_id === 2 && !r.is_deleted);
        if (remote) {
      ...
      if (!remote) {
        if (local.supabaseId) {
          ...
          console.log(`Manga ${local.title} deleted on remote, removing locally.`);
          await db.mediaItems.delete(local.id!);
          await db.mangaMetadata.delete(local.id!);
          continue;
        }
        ...
        if (!local.isDeleted) {
          ...
          if (insertError) console.error('Error inserting manga (media_items):', insertError);
          else {
            await db.mediaItems.update(local.id!, { 
              supabaseId: data.id, 
              lastSyncedAt: new Date() 
            });
            await this.syncMangaMetadata(local.id!, data.id, 'push');
          }
        }
      } else {
        ...
        if (localIsNewer && localHasChanges) {
          ...
          await this.syncMangaMetadata(local.id!, local.supabaseId!, 'push');
          ...
          await this.syncMangaMetadata(local.id!, remote.id, 'pull');
    ...
    // Pull new remote manga
    for (const remote of remoteMediaItems || []) {
      const local = localMediaItems.find(l => l.supabaseId === remote.id || (l.externalId === remote.external_id && l.mediaTypeId === 2));
      if (!local) {
        ...
        await this.syncMangaMetadata(localId as number, remote.id, 'pull');
      }
    }
  }

  private async syncMangaMetadata(localMediaId: number, remoteMediaId: number, direction: 'push' | 'pull' = 'push') {
    const localMeta = await db.mangaMetadata.get(localMediaId);
    ...
    if (direction === 'push' && localMeta) {
      const supabaseData = {
        ...
        publication_status: localMeta.publicationStatus
      };

      if (!remoteMeta) {
        await this.supabase.from('manga_metadata').insert([supabaseData]);
      } else {
        await this.supabase.from('manga_metadata').update(supabaseData).eq('media_item_id', remoteMediaId);
      }
    } else if (direction === 'pull' && remoteMeta) {
      await db.mangaMetadata.put({
        mediaItemId: localMediaId,
        malId: remoteMeta.mal_id,
        authors: remoteMeta.authors || [],
        publishers: remoteMeta.publishers || [],
        publicationStatus: remoteMeta.publication_status,
        progressTotal: remoteMeta.progress_total
      });
      // Update progress and studios (authors) on mediaItem
      await db.mediaItems.update(localMediaId, {
        studios: remoteMeta.authors || [],
      });
    }
  }
}
